...
struct {
  ...
  double offset      = 0.0;
  double value          = 0.0;
  double magick_number  = 123456.789;
} data;
...
void setup() {
  ...
  if (data.magick_number!=123456.789) {
    ...
    strncpy(&(data.wifi.ssid[0]), "FiuNetwork", 32);
//    strncpy(&(data.wifi.ssid[0]), "Sunrise_2.4GHz_8ACA90", 32);
    strncpy(&(data.wifi.password[0]), "K5tY43Rrpdz3", 16);

    strncpy(&(data.mqtt.ip[0]), "192.168.3.236", 16);
    data.mqtt.port=1883;
    
    strncpy(&(data.mqtt.username[0]), "mqtt", 16);
    strncpy(&(data.mqtt.password[0]), "1123581321.Mqtt", 16);
    strncpy(&(data.mqtt.in_topic[0]), "maison/sensor/in", 64);
    strncpy(&(data.mqtt.out_topic[0]), "maison/sensor/out", 64);
